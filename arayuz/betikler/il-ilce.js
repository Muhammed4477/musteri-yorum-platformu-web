...
const IL_ILCE = {
  ...
  "Çorum": ["Alaca","Bayat","Boğazkale","Dodurga","İskilip","Kargı","Laçin","Merkez","Mecitozu","Oğuzlar","Ortaköy","Osmancık","Sungurlu","Uğurludağ"],
  ...
  "Hatay": ["Altınözü","Antakya","Arsuz","Belen","Defne","Dörtyol","Erzin","Hassa","İskenderun","Kırıkhan","Kumlu","Mandası","Payas","Reyhanlı","Samandağ","Serinyol","Yayladağı"],
  ...
  "Samsun": ["Alaçam","Asarcık","Atakum","Ayvacık","Bafra","Canik","Çarşamba","Havza","İlkadım","Kavak","Ladik","Nokta","Ondokuzmayıs","Salıpazarı","Tekkeköy","Terme","Vezirköprü","Yakakent"],
...
// İl listesini alfabetik sırayla döndürür
function getIller() {
  return Object.keys(IL_ILCE).sort();
}

// Seçilen ile göre ilçeleri döndürür
function getIlceler(il) {
  return il && IL_ILCE[il] ? IL_ILCE[il].sort() : [];
}

// <select> elementini il listesiyle doldurur
function fillIlSelect(selectEl, defaultText = 'İl seçin...', selectedIl = '') {
  selectEl.innerHTML = `<option value="">${defaultText}</option>`;
  getIller().forEach(il => {
    const opt = document.createElement('option');
    opt.value = il;
    opt.textContent = il;
    if (il === selectedIl) opt.selected = true;
    selectEl.appendChild(opt);
  });
}

// <select> elementini ilçe listesiyle doldurur
function fillIlceSelect(selectEl, il, defaultText = 'İlçe seçin...', selectedIlce = '') {
  selectEl.innerHTML = `<option value="">${defaultText}</option>`;
  getIlceler(il).forEach(ilce => {
    const opt = document.createElement('option');
    opt.value = ilce;
    opt.textContent = ilce;
    if (ilce === selectedIlce) opt.selected = true;
    selectEl.appendChild(opt);
  });
}
